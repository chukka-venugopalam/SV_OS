...
import { PageTransition, FadeIn, SlideUp, HoverCard } from '@/components/shared/animations';
import { Card, CardContent, CardHeader, CardTitle, Badge, Button, Progress, Skeleton, EmptyState } from '@sv-os/ui';
...
function ContinueLearningCard({ title, description, progress }: { title: string; description: string; progress: number }) {
  return (
      ...
      <CardContent className="p-4">
        <div className="mb-3 flex items-start justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 line-clamp-1">{description}</p>
          </div>
          <PlayCircle className="h-5 w-5 shrink-0 text-primary-500" />
        </div>
        <Progress value={progress} size="sm" />
        <p className="mt-1.5 text-xs text-neutral-400 dark:text-neutral-500">{Math.round(progress)}% complete</p>
      </CardContent>
    </Card>
  );
}
...
interface ActivityItemProps {
  ...
  title: string;
  time: string;
  type: 'started' | 'completed' | 'bookmarked' | 'mastered';
}
...
function ActivityItem({ icon, title, time, type }: ActivityItemProps) {
  ...
  return (
    <div className="flex items-start gap-3 py-3">
      <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full', colors[type])}>
        {icon}
      </div>
      <div className="min-w-0 flex-1">
          ...
          <Card>
            <CardContent className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {activityFeed && activityFeed.items.length > 0 ? (
                activityFeed.items.slice(0, 4).map((a) => (
                  <ActivityItem
                    key={a.id}
                    icon={a.type === 'bookmarked' ? <Bookmark className="h-4 w-4" /> : a.type === 'completed' || a.type === 'mastered' ? <CheckCircle2 className="h-4 w-4" /> : <PlayCircle className="h-4 w-4" />}
                    title={a.node_title}
                ...
                bookmarks.items.slice(0, 3).map((b) => (
                  <ActivityItem
                    key={b.id}
          ...
          <Card>
            <CardContent className="p-4">
              {trendingSearches && trendingSearches.length > 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {['JavaScript', 'React', 'Python', 'Machine Learning', 'System Design', 'Docker'].map((term) => (
                    <Link
                      key={term}
                      href={`${ROUTES.EXPLORE}?search=${encodeURIComponent(term)}`}
                      className="inline-flex items-center gap-1.5 rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-50 hover:text-neutral-900 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                      {learningCount > 0 ? `${learningCount} nodes in progress` : 'Start learning to build your streak'}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-success-50 text-success-600 dark:bg-success-950/30 dark:text-success-400">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                      {completionRate >= 100 ? 'All nodes completed! 🎉' : `${100 - completionRate}% remaining to complete all nodes`}
                    </p>
                  </div>
                </div>
              </CardContent>
