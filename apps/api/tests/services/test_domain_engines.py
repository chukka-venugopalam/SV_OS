...
def patch_domain_engines():
    ...
    with patch('app.services.ai.domain_engines.ContextEngine') as ctx_patch:
        mock_ctx = MagicMock()
        mock_ctx.build_context = AsyncMock(
            return_value={
                'knowledge_graph': {},
                'user_progress': {},
                'activity': [],
        ...
        with patch('app.services.ai.domain_engines.RAGEngine') as rag_patch:
            mock_rag = MagicMock()
            mock_rag.search = AsyncMock(return_value=[])
            rag_patch.return_value = mock_rag

            with patch('app.services.ai.domain_engines.ChatService') as chat_patch:
                ...
                chat_patch.return_value = mock_chat
                yield
...
class TestCareerMentor:
    ...
    async def test_analyse_returns_content(self, mock_uow) -> None:
        ...
        result = await engine.analyse(
            user_id=uuid4(),
            message='What career is right for me?',
        )
        assert 'content' in result
        assert 'suggestions' in result
    ...
    async def test_analyse_with_career_target(self, mock_uow) -> None:
        ...
        result = await engine.analyse(
            user_id=uuid4(),
            message='Am I ready?',
            target_career_slug='data-scientist',
        )
        assert 'content' in result
...
class TestProjectMentor:
    ...
    async def test_mentor_without_tech_stack(self, mock_uow) -> None:
        ...
        result = await engine.mentor(
            user_id=uuid4(),
            project_description='Build anything',
            tech_stack=None,
        )
        assert 'content' in result
