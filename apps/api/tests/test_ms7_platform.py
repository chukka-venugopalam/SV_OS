...
from app.engines.base import EngineBase, EngineState, EngineDependency
from app.engines.graph_engine import GraphEngine
from app.engines.traversal_engine import TraversalEngine
from app.engines.versioning_engine import VersioningEngine, VersionSnapshot, GraphDiff
from app.engines.export_engine import ExportEngine, ExportJob
from app.engines.import_engine import ImportEngine, ImportJob, ImportStage, ConflictReport
...
async def graph():
    ...
    if len(nodes) >= 2:
        await g.add_edge(source_node_id=UUID(nodes[0]['id']), target_node_id=UUID(nodes[1]['id']), relationship_type='prerequisite')
    return g
...
class TestVersioningEngineLifecycle:
    ...
    async def test_snapshot_dataclass(self):
    ...
    async def test_diff_dataclass(self):
...
class TestVersioningEngineSnapshots:
    ...
    async def test_create_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.create_snapshot(notes='Test snapshot', author='tester', tags=['v1.0', 'test'])
    ...
    async def test_get_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_get_snapshot_by_tag(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_get_nonexistent_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.get_snapshot('nonexistent')
        assert result is None

    async def test_list_snapshots(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_list_snapshots_by_branch(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_get_snapshot_full(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
    ...
    async def test_delete_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
    ...
    async def test_immutable_protection(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
        result = await engine.delete_snapshot(created['version_id'])
    ...
    async def test_multiple_snapshots_increment_version(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
...
class TestVersioningEngineRestoreAndRollback:
    ...
    async def test_restore_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()

        # Modify graph
        await graph.add_node(slug='new-node', title='New', node_type='concept', difficulty='beginner')
    ...
    async def test_restore_creates_safety_snapshot(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
        result = await engine.restore_snapshot(created['version_id'])
    ...
    async def test_rollback(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
        await graph.add_node(slug='rollback-test', title='Rollback', node_type='concept', difficulty='beginner')
        result = await engine.rollback(created['version_id'])
        assert result['action'] == 'rollback'
        assert result['success']

    async def test_rollback_validation(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
        await graph.add_node(slug='lost-node', title='Will be lost', node_type='concept', difficulty='beginner')
...
class TestVersioningEngineDiff:
    ...
    async def test_diff_versions(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        v1 = await engine.create_snapshot()
        await graph.add_node(slug='diff-node', title='Diff Node', node_type='concept', difficulty='beginner')
        v2 = await engine.create_snapshot()
    ...
    async def test_diff_identical_versions(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        v1 = await engine.create_snapshot()
        v2 = await engine.create_snapshot()
    ...
    async def test_compare_versions(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        v1 = await engine.create_snapshot()
        v2 = await engine.create_snapshot()
    ...
    async def test_diff_nonexistent_version(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
...
class TestVersioningEngineTagsAndBranches:
    ...
    async def test_tag_version(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
    ...
    async def test_get_version_by_tag(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot(tags=['production'])
        retrieved = await engine.get_version_by_tag('production')
        assert retrieved is not None

    async def test_create_branch(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        created = await engine.create_snapshot()
    ...
    async def test_create_branch_duplicate(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_list_branches(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        branches = await engine.list_branches()
        assert len(branches) >= 1

    async def test_merge_validation(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        await engine.create_snapshot(branch='main')
    ...
    async def test_latest_version(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        v1 = await engine.create_snapshot()
        v2 = await engine.create_snapshot()
    ...
    async def test_version_history(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
        await engine.create_snapshot()
        await engine.create_snapshot()
    ...
    async def test_graph_checksum(self, graph):
        engine = VersioningEngine(graph_engine=graph)
        await engine.initialize()
...
class TestExportEngineLifecycle:
    ...
    async def test_export_job_dataclass(self):
...
class TestExportEngineOperations:
    ...
    async def test_export_graph_json(self, graph):
    ...
    async def test_export_graph_csv(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_export_graph_md(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.export_graph(format='md')
        assert result['format'] == 'md'

    async def test_export_graph_zip(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.export_graph(format='zip')
        assert result['format'] == 'zip'

    async def test_export_subgraph(self, graph, traversal):
    ...
    async def test_export_node(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
        nodes = await graph.all_nodes()
        nid = UUID(nodes[0]['id'])
    ...
    async def test_export_dependency_chain(self, graph, traversal):
        engine = ExportEngine(graph_engine=graph, traversal_engine=traversal)
        await engine.initialize()
        nodes = await graph.all_nodes()
        nid = UUID(nodes[0]['id'])
    ...
    async def test_export_with_filters(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_list_exports(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_get_export_status(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.export_graph(format='json')
    ...
    async def test_export_no_graph(self):
        engine = ExportEngine()
        await engine.initialize()
        result = await engine.export_graph()
        assert 'error' in result

    async def test_export_career_graph(self, graph, traversal):
    ...
    async def test_export_learning_path(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.export_learning_path({'milestones': [{'level': 1, 'nodes': []}]})
        assert result['format'] == 'json'

    async def test_export_assessment(self, graph):
        engine = ExportEngine(graph_engine=graph)
        await engine.initialize()
...
class TestImportEngineLifecycle:
    ...
    async def test_import_job_dataclass(self):
...
class TestImportEnginePipeline:
    ...
    async def test_import_with_valid_payload(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(
            payload={'nodes': [{'id': 'new1', 'slug': 'new', 'title': 'New', 'node_type': 'concept', 'difficulty': 'beginner'}], 'edges': []},
            source='test',
        )
        assert result['stage'] in ('committed', 'failed')

    async def test_import_dry_run(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(
            payload={'nodes': [], 'edges': []},
        ...
        assert result['dry_run_passed'] or result['stage'] == 'dry_run' or result['stage'] == 'preview'

    async def test_import_json_raw(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(
            raw_content='{"nodes": [{"slug": "raw-node", "title": "Raw", "node_type": "concept", "difficulty": "beginner"}], "edges": []}',
            source_format='json',
            source='api',
        )
        assert 'import_id' in result

    async def test_import_csv_raw(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        csv_content = "slug,title,node_type,difficulty\ncsv-node,CSV Import,concept,beginner"
        result = await engine.start_import(
            raw_content=csv_content, source_format='csv', source='test',
        )
        assert 'import_id' in result

    async def test_import_text_format(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        text = "node=My Concept\nnode=Another Concept\nedge=My Concept->Another Concept"
        result = await engine.start_import(
            raw_content=text, source_format='yaml', source='test',
        )
        assert 'import_id' in result

    async def test_import_invalid_json(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(
            raw_content='{invalid json}', source_format='json', source='test',
        )
        assert 'error_message' in result or result.get('validation_errors', 0) > 0

    async def test_import_empty_payload(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(source='test')
        assert 'import_id' in result

    async def test_import_cancellation(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_import_history(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_import_status(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(payload={'nodes': []}, source='test')
        import_id = UUID(result['import_id'])
    ...
    async def test_rollback_import(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(payload={'nodes': []}, source='test')
        import_id = UUID(result['import_id'])
    ...
    async def test_validate_raw_content(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_parse_raw_content(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.parse_raw_content(
            '{"nodes": [{"slug": "parse-test"}], "edges": []}', format='json',
        )
        assert result['nodes'] == 1

    async def test_conflict_detection(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        await graph.add_node(slug='existing-slug', title='Existing', node_type='concept', difficulty='beginner')
    ...
    async def test_preview_import(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
        result = await engine.start_import(payload={'nodes': [{'slug': 'prev'}], 'edges': []}, source='test')
        import_id = UUID(result['import_id'])
        preview = await engine.preview_import(import_id)
        assert 'nodes_to_add' in preview

    async def test_resume_import(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
    ...
    async def test_conflict_report_dataclass(self):
        report = ConflictReport(has_conflicts=True, duplicate_slugs=['slug1'], existing_nodes_overwritten=2)
        assert report.has_conflicts
        assert len(report.duplicate_slugs) == 1
        assert report.new_nodes == 0

    async def test_import_cancel_nonexistent(self, graph):
        engine = ImportEngine(graph_engine=graph)
        await engine.initialize()
...
class TestCrossEngineIntegration:
    ...
    async def test_versioning_export_import_chain(self, graph, traversal):
        """Version → Export → Import chaining."""
        version_eng = VersioningEngine(graph_engine=graph)
        await version_eng.initialize()
        created = await version_eng.create_snapshot()
    ...
    async def test_all_engines_lifecycle(self):
        engines = [
            VersioningEngine(),
            ExportEngine(),
            ImportEngine(),
    ...
    async def test_engine_base_inheritance(self):
        for cls in [VersioningEngine, ExportEngine, ImportEngine]:
            assert issubclass(cls, EngineBase)

    async def test_engine_dependencies_structure(self):
        engines = [
            VersioningEngine(),
            ExportEngine(),
            ImportEngine(),
