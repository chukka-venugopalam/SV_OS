...
from app.engines.base import EngineBase, EngineDependency
...
def build_platform_container() -> PlatformContainer:
    ...
    container.engine_registry.register(
        ...
        dependencies=[EngineDependency('graph', required=True), EngineDependency('traversal', required=True)],
    )
    container.engine_registry.register('state', lambda: StateEngine(), dependencies=[EngineDependency('event', required=False)])
    container.engine_registry.register(
        'recommendation',
        ...
        dependencies=[EngineDependency('graph', required=True), EngineDependency('traversal', required=False), EngineDependency('state', required=False)],
    )
    container.engine_registry.register(
        'learning_path',
    ...
    container.engine_registry.register(
        'assessment',
        lambda: AssessmentEngine(state_engine=container.engine_registry.try_get('state'), graph_engine=container.engine_registry.try_get('graph')),
    ...
    container.engine_registry.register(
        ...
        dependencies=[EngineDependency('graph', required=True), EngineDependency('traversal', required=False)],
    )
    container.engine_registry.register(
        'versioning',
    ...
    container.engine_registry.register(
        'export',
        lambda: ExportEngine(graph_engine=container.engine_registry.try_get('graph'), traversal_engine=container.engine_registry.try_get('traversal')),
        dependencies=[EngineDependency('graph', required=True), EngineDependency('traversal', required=False)],
    )
    container.engine_registry.register('scheduler', SchedulingEngine, dependencies=[EngineDependency('event', required=False)])
    container.engine_registry.register(
        'revision',
        lambda: RevisionEngine(state_engine=container.engine_registry.try_get('state'), graph_engine=container.engine_registry.try_get('graph')),
        dependencies=[EngineDependency('state', required=False), EngineDependency('graph', required=False)],
    )
    container.engine_registry.register(
        'analytics',
        lambda: AnalyticsEngine(graph_engine=container.engine_registry.try_get('graph'), state_engine=container.engine_registry.try_get('state'), cache=container.cache),
        dependencies=[EngineDependency('graph', required=False), EngineDependency('state', required=False)],
    )
    container.engine_registry.register(
        'plugin',
    ...
    container.engine_registry.register(
        'validation',
        lambda: ValidationEngine(graph_engine=container.engine_registry.try_get('graph'), knowledge_engine=container.engine_registry.try_get('knowledge')),
        dependencies=[EngineDependency('graph', required=True)],
    )
    container.engine_registry.register(
        'import',
        ...
        dependencies=[EngineDependency('validation', required=True), EngineDependency('graph', required=True)],
    )
...
def build_platform_container() -> PlatformContainer:
    ...
    container.engine_registry.register(
        'dependency',
        lambda: DependencyEngine(
            graph_engine=container.engine_registry.try_get('graph'),
        ),
        dependencies=[EngineDependency('graph', required=True)],
    )

    # Traversal engine (depends on graph)
    container.engine_registry.register(
        'traversal',
    ...
    # Query engine (depends on graph + traversal)
    container.engine_registry.register(
        'query',
        lambda: QueryEngine(
            graph_engine=container.engine_registry.try_get('graph'),
    ...
    # Versioning engine (depends on graph)
    container.engine_registry.register(
        'versioning',
    ...
    # Export engine (depends on graph + traversal)
    container.engine_registry.register(
        'export',
        lambda: ExportEngine(
            graph_engine=container.engine_registry.try_get('graph'),
    ...
    container.engine_registry.register(
        'plugin',
        lambda: PluginEngine(
            engine_registry=container.engine_registry,
        ),
        dependencies=[EngineDependency('event', required=False)],
    )

    # Validation engine (depends on graph, optionally knowledge)
    container.engine_registry.register(
        'validation',
    ...
    # Import engine (depends on validation + graph)
    container.engine_registry.register(
        'import',
        lambda: ImportEngine(
            validation_engine=container.engine_registry.try_get('validation'),
